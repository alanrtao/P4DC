...
const std::string make_integration_instruction(const hydratable::context& context) {
    ...
    const std::string preemble =
        ...
        "----**To install**----\n"
        "  1. Extract the compressed archive inside your depot (main branch), placing all `.sh` files in a folder named `p4dc-triggers` directly under the workspace folder.\n"
